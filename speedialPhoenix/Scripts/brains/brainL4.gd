		...
		if Manager.getEnemyAmount() == 0:
			match state:
				...
				2:
					...
					player.makeHunSave()
					SignalBus.emit_signal("saveScore")
					Manager.playerRespawnPos = Vector2(1391.0, 1060.0)
					SignalBus.emit_signal("teleporting")
					player.set("finish", true)
					await get_tree().create_timer(.15).timeout
					player.set("finish", false)
					player.position = Vector2(1391.0, 1060.0)
					Manager.levelState = 3
					Manager.playSound("floorBeat", Vector2(1391.0, 1060.0), 10.5)
				3:
					if levelBeat == true:
						SignalBus.emit_signal("makeCamPoint", 0)
